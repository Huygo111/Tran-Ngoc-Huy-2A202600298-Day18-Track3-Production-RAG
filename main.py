...
def main():
    ...
    if not os.path.exists("reports/naive_baseline_report.json") and not os.path.exists("naive_baseline_report.json"):
        print("\n[STEP 1] Running Basic RAG Baseline...")
        print("-" * 40)
        from naive_baseline import main as run_baseline
        run_baseline()
    else:
        print("\n[STEP 1] Skipping Basic RAG Baseline (report already exists).")
